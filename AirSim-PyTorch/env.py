...
class DroneEnv(object):
    ...
    def compute_reward(self, quad_state, quad_vel, collision):
        ...
        if collision:
            ...
            print(f"Diff = {self.last_dist} - {dist}")
            if dist < 5:
                newDest = dest
                while newDest == dest:
                ...
                dest = newDest
                reward += 1000000
            elif self.last_dist != 0:
                reward += diff
            if self.pastDist[49] != 0:
                diff = ((self.pastDist[49] - dist)) ** 3
                reward += diff
                print(f"Past diff = {diff}")
            self.last_dist = dist
            self.pastDist = np.roll(self.pastDist, 1)
            print(self.pastDist)
            self.pastDist[0] = dist
            print(reward)
